...
REMOTE_APP_DIR = 'domains/morsall.com/nodejs' # Updated to the correct directory

ZIP_NAME = "deploy_nodejs.zip"

def create_zip():
    print("Creating main deployment zip...")
    # Zip only necessary production files
    dirs_to_zip = ['.next', '_next', 'public', 'prisma']
    files_to_zip = ['server-hostinger.js', '.env.production', 'package.json', 'next.config.js', 'server.js']
...
def upload_and_deploy():
    ...
    try:
        ...
        cmds = [
            ...
            f"cd {REMOTE_APP_DIR} && rm {ZIP_NAME}"
        ]
        ...
        print("Deployment to 'nodejs' folder finished successfully!")
        client.close()
