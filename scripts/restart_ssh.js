...
console.log('Connecting to Hostinger via SSH to perform a clean restart...');

conn.on('ready', () => {
  console.log('✅ Connected successfully!');
  
  const cmd = `
    echo "=== RUNNING PROCESSES BEFORE KILL ==="
    ps aux | grep -i node
    
    echo "=== KILLING NODE AND LSNODE PROCESSES ==="
    pkill -9 -f node || true
    pkill -9 -f lsnode || true
    sleep 2
    
    echo "=== RUNNING PROCESSES AFTER KILL ==="
    ps aux | grep -i node
    
    echo "=== CLEANING UP OLD ROOT CACHES ==="
  ...
  conn.exec(cmd, (err, stream) => {
    ...
    stream.on('close', (code) => {
      console.log('SSH execution completed with code: ' + code);
      conn.end();
    }).on('data', (data) => {
