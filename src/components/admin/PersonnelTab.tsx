...
export default function PersonnelTab({ type: initialType }: PersonnelTabProps) {
  const [currentType, setCurrentType] = useState<"employees" | "drivers">(initialType || "employees");
  ...
  useEffect(() => {
    setCurrentType(initialType || "employees");
  }, [initialType]);
  ...
  const handleDelete = async (id: string) => {
    if (!confirm(`هل أنت متأكد من حذف ${currentType === 'employees' ? 'الموظف' : 'المندوب'}؟`)) return;
    await fetch(`/api/admin/${currentType}`, { method: "DELETE", body: JSON.stringify({ id }) });
    fetchData();
  };
  ...
  return (
    <div className="space-y-10" dir="rtl">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
         <div className="flex items-center gap-6">
            <div className="w-16 h-16 rounded-[2rem] bg-[#0F172A] text-white flex items-center justify-center shadow-2xl">
               {currentType === 'employees' ? <Users size={32} /> : <Truck size={32} />}
            </div>
            <div>
               <h2 className="text-3xl font-black text-[#0F172A]">إدارة {currentType === 'employees' ? 'الموظفين' : 'المناديب'}</h2>
               <p className="text-gray-400 text-xs font-bold uppercase tracking-widest mt-1">Personnel Management System</p>
            </div>
         </div>
         <button 
           onClick={() => setIsAddOpen(true)}
           className="bg-[#C5A021] text-white px-10 py-5 rounded-[1.5rem] font-black text-sm shadow-xl hover:bg-[#0F172A] transition-all flex items-center gap-3 shrink-0"
         >
            <Plus size={20} />
            إضافة {currentType === 'employees' ? 'موظف' : 'مندوب'} جديد
         </button>
      </div>

      {/* Sub-tabs to switch between employees and drivers */}
      <div className="flex border-b border-gray-200 pb-1 gap-6">
        <button
          onClick={() => setCurrentType("employees")}
          className={cn(
            "pb-3 px-4 text-sm font-bold transition-all relative",
            currentType === "employees" ? "text-[#C5A021] font-black text-base" : "text-gray-400 hover:text-gray-600"
          )}
        >
          <span>الموظفون العامة</span>
          {currentType === "employees" && (
            <motion.div layoutId="personnel-tab-indicator" className="absolute bottom-0 left-0 right-0 h-0.5 bg-[#C5A021]" />
          )}
        </button>
        <button
          onClick={() => setCurrentType("drivers")}
          className={cn(
            "pb-3 px-4 text-sm font-bold transition-all relative",
            currentType === "drivers" ? "text-[#C5A021] font-black text-base" : "text-gray-400 hover:text-gray-600"
          )}
        >
          <span>المناديب وسائقو التوصيل</span>
          {currentType === "drivers" && (
            <motion.div layoutId="personnel-tab-indicator" className="absolute bottom-0 left-0 right-0 h-0.5 bg-[#C5A021]" />
          )}
        </button>
      ...
      {loading ? (
        <div className="p-20 text-center font-black text-gray-400 uppercase tracking-widest animate-pulse">جاري تحميل البيانات...</div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
              ...
              <div key={person.id} className="bg-white p-8 rounded-[3rem] border border-gray-100 shadow-2xl shadow-gray-200/50 group relative overflow-hidden">
                 <div className="relative z-10">
                    ...
                    <div className="flex items-center gap-2 mb-6">
                       ...
                       <p className="text-[10px] font-black text-[#C5A021] uppercase tracking-[0.3em]">{person.role || (currentType === 'employees' ? 'EMPLOYEE' : 'DRIVER')}</p>
                    </div>
           ...
           {data.length === 0 && (
             <div className="col-span-full py-20 text-center text-gray-400 font-bold">لا يوجد مسجلين حالياً في هذا القسم.</div>
           )}
        </div>
      )}
    </div>
